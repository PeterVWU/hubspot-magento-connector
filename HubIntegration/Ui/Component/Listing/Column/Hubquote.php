...
class Hubquote extends Column
{
    ...
    public function __construct(
		...
		array $data = [])
		{
			...
			$this->quoteRepositoryInterface = $quoteRepository;
	...
	public function prepareDataSource(array $dataSource)
	{
		if (isset($dataSource['data']['items'])) {
            foreach ($dataSource['data']['items'] as & $item) {
                $quoteHubId = $this->quoteRepositoryInterface->get($item['entity_id'])->getData('hub_deal_id');
                if ($quoteHubId != null) {
                    $url = 'https://app.hubspot.com/contacts/';
                    $item[$this->getData('name')]['hub_deal_id'] = [
                        'href' => $url.$this->getHubspotPortalId()."/deal/".$quoteHubId,
                    ...
                    $item[$this->getData('name')]['hub_deal_id'] = [
                        'href' => $this->urlBuilder->getUrl(
                            'hubintegration/export/quoteexport',
                            [
